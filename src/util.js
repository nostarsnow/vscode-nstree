...
const util = {
  nstreeFilename: 'nstree.json',
  config: {
    findByFileName: vscode.workspace
      .getConfiguration()
      .get('nstree.findByFileName'),
  },
  getCurrentPath(filepath) {
    filepath = this.formatFilePath(filepath);
    let rootPath = this.getRootPath(filepath);
    let projectDir = this.findUpByFilename(filepath, rootPath);
    return (
      filepath
        .replace(projectDir ? projectDir : rootPath, '')
  ...
  getProjectPath(filepath) {
    filepath = this.formatFilePath(filepath);
    let rootPath = this.getRootPath(filepath);
    let projectPath = this.findUpByFilename(filepath, rootPath);
    return projectPath;
  ...
  formatFilePath(path) {
    ...
    return path.replace(/^\//, '').replace(/^\w/, (word) => word.toUpperCase());
  },
  getRootPath(filepath) {
    filepath = this.formatFilePath(filepath);
    let workspaceFolders = vscode.workspace.workspaceFolders.map((item) =>
      this.formatRootPath(this.formatFilePath(item.uri))
    );
    let rootPath = workspaceFolders.find((v) => filepath.indexOf(v) === 0);
    return rootPath;
  },
  formatRootPath(path) {
    path = path.path ? path.path : path;
  ...
  hasFileNameInDir(dir, filename) {
    ...
    return fs.readdirSync(dir).some((v) => filename.includes(v));
  },
  findUpByFilename(currentPath, rootPath, filename) {
    filename = filename || this.config.findByFileName;
    ...
    if (this.hasFileNameInDir(parentDir, filename)) {
      return parentDir;
    } else {
      return this.findUpByFilename(parentDir, rootPath, filename);
    }
  },
  /**
   * 获取当前工程名
  ...
  openFileInFinder: function (filePath) {
    ...
    if (fs.statSync(filePath).isDirectory()) {
      exec(`explorer.exe ${filePath.replace(/\//g, '\\')}`);
    } else {
      // 如果是文件，要分开处理
